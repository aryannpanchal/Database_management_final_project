...
('Employee',15,1),('Gameday Special',20,1),('Happy Hour',10,1),('Lunch Special Large',2,0),('Lunch Special Medium',1,0),('Specialty Pizza',1.5,0);

INSERT INTO baseprice(baseprice_Size, baseprice_CrustType, baseprice_CustPrice, baseprice_BusPrice) VALUES
('Large','Gluten-Free',9.5,4),('Large','Original',8,2),('Large','Pan',9,3),('Large','Thin',8,1.25),
('Medium','Gluten-Free',6.25,3),('Medium','Original',5,1.5),('Medium','Pan',6,2.25),('Medium','Thin',5,1),
('Small','Gluten-Free',4,2),('Small','Original',3,0.75),('Small','Pan',3.5,1),('Small','Thin',3,0.5),
('XLarge','Gluten-Free',12.5,6),('XLarge','Original',10,3),('XLarge','Pan',11.5,4.5),('XLarge','Thin',10,2);
...
('Bacon',1,1.5,2,3,1.5,0.25,0,89),('Banana Peppers',0.6,1,1.3,1.75,0.5,0.05,0,36),('Black Olives',0.75,1,1.5,2,0.6,0.1,25,39),
('Chicken',1.5,2,2.25,3,1.75,0.25,25,56),('Feta Cheese',1.75,3,4,5.5,1.5,0.18,0,75),('Four Cheese Blend',2,3.5,5,7,1,0.15,25,150),
('Goat Cheese',1.6,2.75,4,5.5,1.5,0.2,0,54),('Green Pepper',1,1.5,2,2.5,0.5,0.02,25,79),('Ham',2,2.5,3.25,4,1.5,0.15,25,78),
('Jalapenos',0.5,0.75,1.25,1.75,0.5,0.05,0,64),('Mushrooms',1.5,2,2.5,3,0.75,0.1,50,52),('Onion',1,1.5,2,2.75,0.5,0.02,25,85),
('Pepperoni',2,2.75,3.5,4.5,1.25,0.2,50,100),('Pineapple',1,1.25,1.75,2,1,0.25,0,15),('Regular Cheese',2,3.5,5,7,0.5,0.12,50,250),
('Roma Tomato',2,3,3.5,4.5,0.75,0.03,10,86),('Sausage',2.5,3,3.5,4.25,1.25,0.15,50,100);

INSERT INTO customer(customer_FName, customer_LName, customer_PhoneNum) VALUES
('Andrew','Wilkes-Krier','8642545861'),('Frank','Turner','8642328944'),('Matt','Engers','8644749953'),('Milo','Auckerman','8648785679');
...
(NULL,@ord3,'Large','Original','completed',@dt3,14.88,3.30),(NULL,@ord3,'Large','Original','completed',@dt3,14.88,3.30),
(NULL,@ord3,'Large','Original','completed',@dt3,14.88,3.30),(NULL,@ord3,'Large','Original','completed',@dt3,14.88,3.30),
(NULL,@ord3,'Large','Original','completed',@dt3,14.88,3.30),(NULL,@ord3,'Large','Original','completed',@dt3,14.88,3.30);
...
SELECT @p2 := @p1 + 1; SELECT @p3 := @p1 + 2; SELECT @p4 := @p1 + 3; SELECT @p5 := @p1 + 4; SELECT @p6 := @p1 + 5;
...
SELECT @p15 := @p14 + 1; SELECT @p16 := @p14 + 2;
...
INSERT INTO pizza_topping VALUES (@p8,(SELECT topping_TopID FROM topping WHERE topping_TopName='Regular Cheese'),1);
...
INSERT INTO pizza_topping VALUES (@p10,(SELECT topping_TopID FROM topping WHERE topping_TopName='Pepperoni'),1);
...
INSERT INTO pizza_topping VALUES (@p7,(SELECT topping_TopID FROM topping WHERE topping_TopName='Four Cheese Blend'),1);
...
INSERT INTO pizza_topping VALUES (@p15,(SELECT topping_TopID FROM topping WHERE topping_TopName='Pineapple'),1);
