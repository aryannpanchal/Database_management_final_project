...
public class DBNinja {
    ...
    public static void addOrder(Order o) throws SQLException, IOException {
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(insertOrder, Statement.RETURN_GENERATED_KEYS)) {
                ...
                // Menu passes Timestamp.toString(); parse it back
                Timestamp ts;
                try {
            ...
            if (o.getOrderType().equals(dine_in) && o instanceof DineinOrder) {
                ...
                // For pickup we track pickup_IsPickedUp
                ...
                // Address format: "houseNum\tstreet\tcity\tstate\tzip"
                if (addr != null) {
    ...
    public static ArrayList<Order> getOrdersByDate(String date) throws SQLException, IOException {
        ...
        ArrayList<Order> orders = new ArrayList<>();
        String sql =
                "SELECT * FROM ordertable WHERE DATE(ordertable_OrderDateTime) = ? ORDER BY ordertable_OrderID ASC";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, date);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    // IMPORTANT: constructor is (pizzaID, size, crustType, ...)
