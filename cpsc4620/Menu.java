...
public class Menu {
    ...
    public static void MarkOrderAsComplete() throws SQLException, IOException {
        ...
        if (tempO != null) {
            // For the menu option, we just mark it complete in the DB.
            // DBNinja.completeOrder will take care of setting flags based on order type and state.
    ...
    public static void AddInventory() throws SQLException, IOException {
        ...
        // interpret the number as the actual TopID (to match how inventory is displayed)
        Topping chosen = null;
        for (Topping t : curInventory) {
            if (t.getTopID() == chosen_t) {
                chosen = t;
                break;
            }
        }
        ...
        if (chosen != null) {
            ...
            DBNinja.addToInventory(chosen.getTopID(), add);
        } else {
    ...
    public static Pizza buildPizza(int orderID) throws SQLException, IOException {
        ...
        while (TopID != -1) {
            ...
            if (myTop == null) {
                System.out.println("That is not a valid topping ID...");
            } else {
    ...
    // Uses actual TopID instead of list index
    ...
    // Uses actual DiscountID instead of list index
    private static void printDiscounts(ArrayList<Discount> discs) {
        ...
        for (Discount d : discs) {
            String amtStr = (d.isPercent() ? "" : "$") + d.getAmount() + (d.isPercent() ? "%" : "");
            System.out.printf("%-5s%-25s%-20s%n", d.getDiscountID(), d.getDiscountName(), amtStr);
        }
    }
    ...
    // NO CODE SHOULD BE PLACED BELOW THIS LINE
